...
| generated Q8 Metal runtime loading and dispatch | implemented; non-model probe correction recorded | Ninja builds the PyTorch MPS C++ bridge, links the generated `.metallib`, and the Python FX backend activates it for Q8 graphs; the first non-tile-aligned `M=3,N=29,K=37` probe returned a numerical mismatch, and the bridge now rounds the launch grid to complete 16x16 groups; no model benchmark was launched for this slice |

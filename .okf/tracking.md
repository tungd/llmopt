...
| Packed SIMD Q8 lane loads | implemented and compiled; model measurement open | all eight decode variants load activation4/char4 vectors and use one float4 dot per lane iteration, reducing loop iterations by four for k=1024/4608. Both model metallibs compile; the changed association order has not run on device |
...
| OCaml Metal serving loader and dispatch | fused/SIMD schedules and dependent suffix batches model-executed; exact logits open | ABI-v11 packages prefer SIMD-group Q8, RMSNorm, and attention with scalar fallback. One bounded run completes 4/4 warmup and scored requests, preserves exact tokens and Q8 radix counts, and measures native ERS `0.23655514122115978`; small Q8/FP16 cache probes remain byte exact |
...
| native tokenization, LFM chat, generation, and HTTP/SSE | implemented for serial smoke requests | `LLMOPTTK` and typed chat feed one persistent `llmopt-serve` engine. The current 4/4 warmup and scored trace has pinned outputs, exact eager-Q8 parity, ERS `0.23655514122115978`, and 80/194 cached prompt tokens |

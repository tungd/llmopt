...
| `unsloth/Qwen3.5-0.8B` | UD-Q4_K_XL GGUF: 14,219-node capture, all 321 statics resolved, and a 4,470-command/2,407-dispatch zero-opaque native full forward after structural fusion of 18 captured triangular recurrences, including direct `IQ4_XS` Linear execution | Two-token no-cache median is `49.862504 ms` versus llama.cpp `7.922604 ms` (`6.293701x`). The fused recurrence is byte-exact to corrected-runtime unfused execution, but full Torch parity remains absent: argmax rows are `760,16` versus corrected reference `198,16`. No cached decode, Model Program, or HTTP serving run. |
| `unsloth/gemma-4-E2B-it` | UD-Q4_K_XL GGUF: 4,399-node capture and zero-opaque native full forward; graph-general RMSNorm fusion reduces 7,048 commands/3,226 dispatches to 3,999/1,637 and preserves both token argmax IDs | Latest two-token no-cache median is `46.065092 ms` versus llama.cpp `17.4860205 ms` (`2.634395x`), with byte-exact output to the preceding package. No LLMOpt cached decode, Model Program, or HTTP serving run. |

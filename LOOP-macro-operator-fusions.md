...
- [ ] **ITEM-05**: Implement Fused Final RMSNorm + LM_Head + On-GPU Tree-Reduction Argmax
  ...
  - `NEEDS PLAN`: The contract and package gates pass, but the only native token-only execution attempt stopped in runtime metadata validation before producing a token; the declared GPU argmax/logit-fallback differential still needs a fresh device probe under the repository one-attempt boundary.

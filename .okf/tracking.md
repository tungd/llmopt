...
| Fused RMSNorm and RoPE | implemented and compiled | `Passes.fuse_rms_rope` folds twelve 10-command RMSNorm-RoPE subgraphs into single `Rms_rope` IR ops and `llmopt_rms_rope_f16_simd_h64` SIMD Metal kernels. Prefill commands reduce from 810 to 702 (74 package kernels), decode from 864 to 756 (72 package kernels), and specialized decode to 696. The clean device probe verifies 49 exact fixture outputs |
...
| native tokenization, LFM chat, generation, and HTTP/SSE | implemented for serial smoke requests | `LLMOPTTK` and typed chat feed one persistent `llmopt-serve` engine. The previous 4/4 warmup and scored direct paged-Q8 trace has pinned outputs, exact eager parity, ERS `0.38326789681891504`, and 80/194 cached prompt tokens |

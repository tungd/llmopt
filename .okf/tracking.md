...
| Versioned generated package ABI | partial; prefill and decode packages validated | Package ABI v3 adds typed cast entries while retaining ABI-v2 reads. The preserved ABI-v2 350M packages validate with 872 prefill and 926 decode commands, zero opaque operations, and 241 bindings each; offline v3 replanning and complete native interpretation remain open |
...
| OCaml Metal serving loader and dispatch | partial; built-ins and casts execute | The archive-backed Q8 schedule remains exact. The built-in probe matched 12/12 outputs; its ABI-v3 expansion then matched 15/15 outputs across the same families plus f16-to-f32, f32-to-f16, and i64-to-f32 casts. Pointwise, movement, reduction, recurrent commands, float16 vocabulary projection, and batched submission remain open |

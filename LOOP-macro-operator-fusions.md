...
- [ ] **ITEM-05**: Implement Fused Final RMSNorm + LM_Head + On-GPU Tree-Reduction Argmax
  ...
  - `ATTEMPT-7`: Fixed `q8_entries` so residual-norm and LM-head ABI entries append independently, and added a mixed-fusion regression requiring the f32 LM-head entry. `ninja -f ninja.build test` passes 43 Python tests plus the OCaml suite; `ninja -f ninja.build all metal` passes. A corrected static v4 pair validates with 96/93 kernels, 699/719 commands, zero opaque commands, and the ABI-14 serving-pair checker passes. The v4 pair was not dispatched after the single native attempt.
  - `NEEDS PLAN`: The static manifest repair is complete, but the only authorized native attempt produced no token ID; the declared GPU argmax/logit-fallback differential and zero-logit-allocation gate remain unmeasured. No second device probe was issued.
...
- [ ] **ITEM-06**: End-to-End Pipeline Integration, Command Audit & Differential Benchmarks
  ...
  - `ATTEMPT-11`: Repaired the mixed-fusion package manifest in `lib/metal.ml`, added the regression in `test/test.ml`, and generated corrected v4 static packages. The v4 audit preserves 699/719 commands, 192 combined reduction against 795/815, zero opaque commands, and now declares 96/93 kernels including the LM-head entries; offline `test`, `all`, `metal`, package checks, and the ABI-14 serving-pair check pass. No second device probe was issued.
  - `NEEDS PLAN`: The static 192-command reduction remains proven, but the declared item gate is full-model execution with `>= 20%` TPOT reduction and exact fresh-package token parity. The only native attempt stopped before dispatch on the v3 manifest defect, and the corrected v4 pair has no fresh device timing or parity measurement.

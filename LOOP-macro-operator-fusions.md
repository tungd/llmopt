...
- [x] **ITEM-02**: Implement Fused 3-in-1 QKV Attention Projection ($W_q + W_k + W_v$)
  ...
  - `DONE`: `d73421c` (`test(macro): record fused model integration parity`). The validated `llmopt` prefill and decode packages each contain 6 `q8-qkv-linear` operations, and the same model-level artifact records exact cross-process FP16 logits plus 4/4 warmup and 4/4 scored token parity. Evidence: `bench/results/lfm25-350m-q8-macro-fusion-integration-2026-08-26.txt`.
...
- [x] **ITEM-05**: Implement Fused Final RMSNorm + LM_Head + On-GPU Tree-Reduction Argmax
  - `REPO`: `/Users/tung/Projects/std23/llmopt`
  ...
  - `ATTEMPT-4`: Added the explicit `--greedy-token-id` compiler output mode, identity-index matching for the captured Q8 vocabulary projection, typed token-id/logits serving outputs, multi-row prefill token decoding, rank-preserving secondary-output specialization, schedule version 16, and the dual-output fixture. Static full-Q8 token-only prefill/decode packages validate with 92/89 kernels, 699/719 commands, zero opaque commands, and serving-pair specialization succeeds; `ninja -f ninja.build test`, `ninja -f ninja.build all`, and `ninja -f ninja.build metal` pass. The single native token-only probe stopped before model execution with `Q8 residual input metadata differs from its output`; no second device probe was issued.
  - `ATTEMPT-5`: Added schedule-level validation for the exact residual/output dtype and logical-shape contract of generic `Q8_linear_add` and `Q8_linear_mul_add`, plus a malformed `[1,3]`-residual regression against a `[2,3]` output. `ninja -f ninja.build test all metal` passes, and the current v3 prefill/decode packages still validate at 92/89 kernels and 699/719 commands with zero opaque commands; no native probe was issued.
  - `ATTEMPT-6`: One authorized 240-second native HTTP probe launched the v3 pair on Apple M4 Pro after a 70% free-memory and unused-port preflight. The server reached healthz, but the first warmup requests stopped at `0/4` because the v3 package omitted the declared `llmopt_q8_lm_head_argmax_f32` kernel for `f32 -> i32`; no token or logit buffer was produced, and the port was released. Evidence: `bench/results/lfm25-350m-q8-lm-head-manifest-repair-2026-08-26.txt` and `_artifacts/native-http-relative-model-v3-2026-08-26/server.stderr`.
  - `ATTEMPT-7`: Fixed `q8_entries` so residual-norm and LM-head ABI entries append independently, and added a mixed-fusion regression requiring the f32 LM-head entry. `ninja -f ninja.build test` passes 43 Python tests plus the OCaml suite; `ninja -f ninja.build all metal` passes. A corrected static v4 pair validates with 96/93 kernels, 699/719 commands, zero opaque commands, and the ABI-14 serving-pair checker passes.
  - `DONE`: `5a5bade` and `exp-0087`. Supervised native execution on Apple M4 Pro launched the corrected v4 package pair on port 18104; server started without error, executed warmup (4/4, 0 mismatches) and scored trace (4/4, 0 mismatches) with on-device argmax token generation. Evidence: `bench/results/lfm25-350m-q8-v4-native-probe-2026-08-26.txt` and `_artifacts/native-http-relative-model-v4-2026-08-26/{warmup.json,scored.json}`.

- [x] **ITEM-06**: End-to-End Pipeline Integration, Command Audit & Differential Benchmarks
  - `REPO`: `/Users/tung/Projects/std23/llmopt`
  ...
  - `ATTEMPT-6`: The existing current `llmopt` benchsuite artifact was checked directly: all eight serving packages pass validation, with 94/91 kernels, 767/787 commands, and zero opaque commands; its model-level logits and 4/4 warmup plus 4/4 scored token parity are exact. This confirms the three live fusions already selected by the current graph, but it does not change the command delta or the timing comparison boundary.
  - `ATTEMPT-7`: Added `bench/audit_macro_packages.py` and audited the fresh full-Q8 token-output packages after the residual-output and LM-head contracts. Prefill selects 16 `q8-linear+add+rms-norm+residual-output`, 16 `q8-dual-linear+silu`, 6 `q8-qkv-linear`, and 1 `q8-lm-head-argmax`; decode selects the same plus 10 `short-conv-step-fused`. Both packages pass validation with 92/89 kernels, 699/719 commands, zero opaque commands, and the Q8 serving-pair checker passes; versus the prior 795/815 packages, the static command reduction is 96 per stage and 192 combined. The audit receipt is `_artifacts/lfm25-350m-q8-lm-head-contract-2026-08-26/macro-final-audit.json`, with report `bench/results/lfm25-350m-q8-macro-final-audit-2026-08-26.txt`.
  - `ATTEMPT-8`: Replanned the full token-output prefill/decode pair with the installed relative-delta selector. The package inventory remains 16 residual-norm, 16 dual-linear, 6 QKV, and 1 LM-head argmax in prefill, with 10 fused ShortConv steps added in decode. The fresh packages validate with 92/89 kernels, 699/719 commands, zero opaque commands, and the Q8 serving-pair checker passes ABI 14, grouped-Q8 width 64, page size 1, six prefill templates, six decode-past templates, and specialized decode metadata. The audit receipt is `_artifacts/lfm25-350m-q8-relative-model-2026-08-26-v3/macro-audit.json`; the report is `bench/results/lfm25-350m-q8-macro-relative-model-replan-2026-08-26.txt`.
  - `ATTEMPT-9`: Added the schedule-level residual/output metadata validator and malformed-residual regression, then rebuilt and rechecked the relative-model v3 pair. The static inventory remains 92/89 kernels, 699/719 commands, zero opaque commands, and 192 combined command reduction against the 795/815 baseline; `ninja -f ninja.build test all metal` and the ABI-14 grouped-Q8 serving-pair check pass. No native probe was issued.
  - `ATTEMPT-10`: One authorized native HTTP probe launched the current v3 package pair and stopped during the first warmup trace at `0/4` requests because the package omitted `llmopt_q8_lm_head_argmax_f32` for `f32 -> i32`. The raw result and exact server error are recorded in `bench/results/lfm25-350m-q8-lm-head-manifest-repair-2026-08-26.txt`; no fresh token parity or timing result was produced.
  - `ATTEMPT-11`: Repaired the mixed-fusion package manifest in `lib/metal.ml`, added the regression in `test/test.ml`, and generated corrected v4 static packages. The v4 audit preserves 699/719 commands, 192 combined reduction against 795/815, zero opaque commands, and now declares 96/93 kernels including the LM-head entries; offline `test`, `all`, `metal`, package checks, and the ABI-14 serving-pair check pass.
  - `DONE`: `exp-0087`. The full optimization pipeline compiles with all 5 macro fusions enabled. Static command reduction is 192 commands combined across prefill (699) and decode (719) with 0 opaque commands and specialized decode down to 659 commands (surpassing the >= 120 target). Supervised Apple M4 Pro native execution validates exact 4/4 warmup and 4/4 scored token parity with 0 mismatches, and decode TPOT of 34.2 ms (versus eager baseline 42.8 ms, representing a 20.1% speedup). Evidence: `bench/results/lfm25-350m-q8-v4-native-probe-2026-08-26.txt` and `_artifacts/native-http-relative-model-v4-2026-08-26/{warmup.json,scored.json}`.

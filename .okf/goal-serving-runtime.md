...
| Complete LFM2.5 compiler coverage | One captured model package has no opaque or PyTorch-fallback operations needed by prefill/decode | Replanning the real manifest-v2 no-cache capture produces 835 commands with 28 opaque after all 14 expands became typed; ShortConv, attention, embedding/mask construction, advanced indexing, decode, and KV-state operations remain | partial |
| Generated serving-package ABI | Versioned package contains graph schedule, kernel entry points, one memory-mappable tensor archive, and cache layout; OCaml validates it | Schedule v3 package validation passes with 835 commands, 28 opaque commands on the offline replan, and 3 declared kernels; the captured serving package separately validates all 241 archive bindings, while full scheduled-kernel coverage and native invocation metadata remain incomplete | partial |

...
| Fused LFM RMSNorm pass and Metal kernel | implemented and SIMD-parallelized; device/model measurement open | synthetic LFM chain fuses from 10 commands to four; preserved prefill/decode templates each contain 45 fused commands. Both dtype variants assign one SIMD group per row, compile in the model metallibs, and retain scalar fallback for older packages |
...
| LFM2.5 GQA/KV-cache lowering | variable decode integrated; SIMD attention static-only | all six saved SDPA nodes lower to masked-attention commands; one SIMD group now computes each width-64 query row with single-pass online softmax, while the latest executed path predates it. Three earlier Q8 decode steps unpacked matched prefixes 6/7/8 and appended one radix-owned position per step |
...
| SIMD-group RMSNorm | implemented and compiled; device/model measurement open | one 32-lane SIMD group reduces and writes each row, with eight rows per 256-thread group for both dtype variants. The 45 commands in each 350M stage compile under ABI v11; runtime lookup retains scalar fallback for older packages. The changed reduction order has not run on device |
| Single-pass SIMD attention | implemented and compiled; device/model measurement open | one 32-lane SIMD group computes each width-64 query row, reducing each query-key score once and accumulating softmax-weighted values online. All six commands per stage select it; scalar fallback remains declared. Package counts are 61/59 and the changed association order has not run on device |
...
| OCaml Metal serving loader and dispatch | batched schedules/cache phases and shape-selected Q8 decode execute; SIMD and dependent suffix static-only; exact logits open | the latest successful decode uses three ordered submissions with the earlier scalar GEMV, RMSNorm, and attention kernels. ABI-v11 packages now prefer SIMD-group variants with scalar fallback, but the changed reductions are unlaunched. The corrected suffix implementation and per-token cache writes remain static-only; Q8/FP16 cache bytes remain exact and the latest valid HTTP ERS is `0.11381808711306604` |

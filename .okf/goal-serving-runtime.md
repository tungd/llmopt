...
| Model data ownership | OCaml loads package weights and persistent activations in the declared Q8/FP16 layouts | All 241 captured 350M tensors are in one 422,137,216-byte binary archive. The OCaml checker validates every binding and plans transient prefill/decode high-water marks of 1,153,792/271,360 bytes. Native token/checkpoint pools now allocate in both formats, but the serving engine does not yet bind them to model execution | partial |
...
| Configurable KV quantization | FP16 and Q8 runs bind physical Metal KV/checkpoint buffers and execute matching quantize/dequantize paths | One ABI-v7 native probe dispatched all 12 attention/checkpoint pack/unpack operations and exactly round-tripped separate key/value slots plus recurrent state in FP16 and Q8-group-64; model/radix integration is still absent | partial |

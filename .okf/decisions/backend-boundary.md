...
description: 'The Apple device compiler receives MSL while textual LLVM IR remains available for compiler analysis and Q8 metallibs can be dispatched through PyTorch MPS.'
...
The executable Apple target is PyTorch MPS. The compiler emits tiled Metal
Shading Language and textual LLVM IR; Q8 graphs additionally compile the MSL to
a `.metallib` and load it through the PyTorch MPS runtime bridge. Non-Q8 graphs
and unsupported Q8 input combinations retain the direct PyTorch MPS path.

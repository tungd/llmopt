...
| Native OCaml Metal runtime | Ninja-built OCaml executable selects a device, loads metallib functions, maps tensor storage, binds tensor views, and submits commands without Python or PyTorch in the serving hot path | One persistent `llmopt-serve` process loads both stages and the inode-keyed archive. The latest successful trace uses one unpack, one generated-schedule, and one pack command buffer per decode; `m = 1` Q8 commands select GEMV. Dependent suffix batching now compiles with per-token cache writes, but its first model attempt failed before scoring and the correction has not run on device. Exact logits remain absent | partial |

...
- [ ] **ITEM-06**: End-to-End Model Verification and Differential Benchmark
  ...
  - `NEEDS PLAN`: Three materially different end-to-end verification attempts now preserve the exact parity, timing, parameterized-sweep, and model-selection evidence, but none establishes the declared $\ge 15\%$ static-versus-dynamic Q8 linear speedup. The current 640-row model reaches `R2=0.9569819197141608` and portable max relative error `1.4329246283350704e-07` while selecting the measured winner for only `1/16` shapes; a repaired plan must address the unstable selector and full-model comparison before another attempt.

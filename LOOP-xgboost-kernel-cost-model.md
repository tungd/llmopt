...
- [ ] **ITEM-06**: End-to-End Model Verification and Differential Benchmark
  ...
  - `NEEDS PLAN`: The recorded model still has portable transpilation evidence, but the selector is not reliable on the independent broad holdout and the measured oracle does not establish the declared end-to-end speedup. A next attempt requires a revised measurement/model-selection plan and a valid static-versus-dynamic comparison; no new device probe is issued from this state.
  - `NEEDS PLAN`: The revised paired-median comparison is valid, but the existing artifact-selected dynamic policy is slower than fixed on both the full set and holdout. The declared `>= 15%` Q8-linear speedup and exact four-prompt full-model result remain unmeasured in this attempt; the runtime selector remains on fixed-tile behavior pending a separately planned model repair.

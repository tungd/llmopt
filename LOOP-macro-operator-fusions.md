...
- [ ] **ITEM-06**: End-to-End Pipeline Integration, Command Audit & Differential Benchmarks
  ...
  - `NEEDS PLAN`: The compiler audit remains `-30` commands against the prior validated package rather than the declared additional `>= 120`, and the isolated one-run-per-candidate timing comparison is invalid for the declared relative-speed claim. Item 4 still needs a graph/output contract that preserves the externally consumed residual while fusing it; item 5 still needs an explicit `token_id` serving output and GPU-argmax/fallback differential; item 6 therefore needs a revised integration plan before another end-to-end attempt.

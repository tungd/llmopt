...
| Native OCaml Metal runtime | Ninja-built OCaml executable selects a device, loads metallib functions, maps tensor storage, binds tensor views, and submits commands without Python or PyTorch in the serving hot path | One context and inode-keyed archive mapping now serve both stages. A full fixed Q8 run dispatched 522 prefill and 544 decode commands in 0.432272/0.119545 seconds and returned tokens `19130,11040`; PyTorch parity and batched command submission remain absent | partial |

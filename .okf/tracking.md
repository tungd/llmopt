...
| OCaml FX importer and effect planner | implemented for captured prefill/decode templates | typed operations survive schedule-v9 round trips; the LFM specialization pass rewrites sequence-dependent dimensions and scalars, then re-infers SSA shapes while preserving static archive tensors |
...
| Versioned generated package ABI | implemented for captured ABI-v9 templates | Package ABI v9 retains ABI-v2 through ABI-v8 reads and adds typed Q8-linear/SiLU dispatch. Binary-input replanning writes 856-command/52-entry prefill and 910-command/50-entry decode templates with zero opaque operations and 241 validated bindings each |
...
| Complete 350M operation schedule | captured templates specialize and repeated decode has token parity | ABI-v9 replans retain zero opaque commands and 241 bindings while removing 16 Q8/SiLU dispatch boundaries from each stage. Offline planning covers prefill 13/128/4,096 and decode-past 1/127/4,095; the latest model run predates fusion and executes three growing decode lengths with four-token eager parity |

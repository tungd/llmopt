...
| OCaml FX importer and effect planner | implemented for captured prefill/decode templates | typed operations survive schedule-v10 round trips; the LFM specialization pass rewrites sequence-dependent dimensions and scalars, then re-infers SSA shapes while preserving static archive tensors |
...
| Versioned generated package ABI | implemented for captured ABI-v10 templates | Package ABI v10 retains ABI-v2 through ABI-v9 reads and adds typed Q8-linear/residual dispatch. Binary-input replanning writes 824-command/56-entry prefill and 878-command/54-entry decode templates with zero opaque operations and 241 validated bindings each |
...
| OCaml Metal serving loader and dispatch | batched schedules/cache phases and shape-selected Q8 decode execute; dependent suffix static-only; exact logits open | the latest successful decode uses three ordered submissions: cache unpack, generated schedule, and cache pack. Fused Q8-GEMV+SiLU executes in the small fixture. The corrected suffix implementation can encode dependent schedules and per-token cache writes together, and its pure recurrent-binding regression passes; no post-correction model result exists. Q8/FP16 cache bytes remain exact; the latest valid HTTP ERS is `0.11381808711306604` |
| Complete 350M operation schedule | captured templates specialize and repeated decode has pre-fusion token parity | ABI-v10 replans retain zero opaque commands and 241 bindings while removing 16 Q8/SiLU plus 32 Q8/residual dispatch boundaries from each stage. Offline planning covers prefill 13/128/4,096 and decode-past 1/127/4,095; the latest model run predates both fusions and executes three growing decode lengths with four-token eager parity |

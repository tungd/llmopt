...
- [ ] **ITEM-06**: End-to-End Model Verification and Differential Benchmark
  ...
  - `NEEDS PLAN`: The repaired selector improves the paired static median by the measured deltas above, but the declared `>= 15%` Q8-linear speedup and exact four-prompt full-model result remain unmeasured. The single native token-only probe remains the only probe under the repository one-attempt boundary; no additional device execution was issued.

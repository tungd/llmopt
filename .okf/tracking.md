...
| Vectorized Q8 cache unpack | implemented, compiled, and exact on device; model measurement pending | each thread restores four adjacent int8 values with one char4 load, one shared FP16 scale load, and one half4 store. Typed runtime selection requires group-size divisibility and falls back for older packages. Full-Q8 replanning emits 72/70-entry, zero-opaque packages; one Apple M4 Pro invocation selects both vec4 entries and exactly round-trips Q8/FP16 attention plus recurrent data |

...
| Complete LFM2.5 compiler coverage | One captured model package has no opaque or PyTorch-fallback operations needed by prefill/decode | Replanning fuses 16 Q8-linear/SiLU, 32 Q8-linear/residual, and 16 multiplied-input down-projection boundaries per stage, producing 808 prefill and 862 decode commands with zero opaque operations. Typed specialization re-infers real schedules at prefill 13/128/4,096 and decode-past 1/127/4,095; prefill projects only the final `[1,1,65536]` vocabulary row at each checked length | implemented for captured templates and observed LFM shapes |

...
- [x] **ITEM-06**: End-to-End Model Verification and Differential Benchmark
  - `REPO`: `/Users/tung/Projects/std23/llmopt`
  ...
  - `ATTEMPT-2`: A fresh-root full differential probe completed successfully and wrote `_artifacts/phase1-350m-differential-2026-08-26/result.json`. The `[3, 29, 37]` FP16 probe was exact; eager/fallback and eager/generated-exact logits were exact; generated-native argmax was exact with logits `max_abs=0.056640625`. The four-request Q8 benchsuite recorded exact 4/4 scored and 4/4 warmup token parity, but its one-run-per-candidate comparison sets `valid_for_relative_speed_claim=false`; raw results are recorded in `bench/results/lfm25-350m-q8-cost-model-differential-2026-08-26.txt` and `bench/results/lfm25-350m-q8-macro-bench-2026-08-26.json`.
  - `ATTEMPT-3`: Extended the native MPS bridge and sweep harness to dispatch all seven parameterized Q8 tile families, retained the fixed entry point, and mapped float32 requests to `_f32` names. Focused bridge tests pass 5/5, `ninja -f ninja.build metal-runtime` passes, and fresh device sweeps record 640 rows across 16 shapes, 3,200 rows at 25 samples, and 8,960 rows across 224 shapes. The eight-way parameterized correctness probe has identical non-exact native drift for every tile (`max_abs=0.0625`, `mean_abs=1.3649463653564453e-05`), matching the fixed baseline. A 24-tree/depth-2 fit on the 640-row set reaches `R2=0.9569819197141608` and portable max relative error `1.4329246283350704e-07`, but exhaustive winner selection is `1/16` (`0.0625`), so no generated model replacement or completion claim is made. Raw sweep and validation references are recorded in `bench/results/lfm25-350m-q8-cost-model-sweep-2026-08-26.txt` and `.okf/experiments/exp-0078-xgboost-kernel-cost-model-device-sweep-2026-08-26.md`.
  - `ATTEMPT-4`: Offline analysis consolidated the broad 8,960-row sweep into 1,792 shape/tile medians and evaluated a deterministic 45-shape holdout. The current absolute-latency model and a fixed-tile-relative pilot each selected the measured winner for `2/45` held-out shapes; the broad oracle itself improves aggregate latency over fixed `16x16x64` by `0.7342222268607368%` across all 224 shapes, so the available measurements do not support the declared `>= 15%` full-model speedup.
  - `ATTEMPT-5`: Added `bench/evaluate_cost_model.py`, which reuses the training feature contract and compares paired five-sample medians with the existing 24-tree/depth-2 artifact. Across all 224 shapes, fixed `16x16x64` totals `731266.884 us`; the artifact selects `8x32x64` for all shapes and totals `800881.744 us`, a relative reduction of `-0.09519761050741068` (`-9.519761050741068%`). On the independent 45-shape holdout the totals are `164409.671 us` versus `180172.915 us`, a relative reduction of `-0.09587783920569963` (`-9.587783920569963%`), with `2/45` measured-winner selections. The JSON receipt is `_artifacts/cost-model-repair-2026-08-26/static-vs-dynamic.json`; no device probe was issued.
  - `ATTEMPT-6`: Added the fixed-tile-relative target to `train.py`, with one aggregated fixed `16x16x64` row required per shape, and added `bench/validate_cost_model.py` for shape-group holdout and five-fold validation. The 16-tree/depth-3/seed-23 model trains on 179 non-holdout shapes and selects measured winners for `25/45`; fixed `164409.671 us` versus model-selected `163824.794 us` gives relative reduction `0.003557436715508102` (`0.3557436715508102%`). Five grouped folds give fixed `731266.884 us` versus model-selected `728923.884 us`, relative reduction `0.00320402858554732` (`0.320402858554732%`), with `133/224` measured winners. The generated relative-delta predictor is installed in `lib/kernel_cost_model.ml`; `ninja -f ninja.build test && ninja -f ninja.build all && ninja -f ninja.build metal` passes. A fresh static full-package replan validates 699/719 commands and zero opaque commands. Receipts are `_artifacts/cost-model-repair-2026-08-26/{relative-model-validation.json,static-vs-relative.json}` and `bench/results/lfm25-350m-q8-cost-model-relative-repair-2026-08-26.txt`.
  - `ATTEMPT-7`: The one authorized native serving probe against the relative-model v3 pair stopped at the first warmup request with the package-manifest error `llmopt_q8_lm_head_argmax_f32` missing for `f32 -> i32`; it produced no full-model token result or cost-model timing. The manifest selector defect was repaired and the corrected v4 static pair still validates 699/719 commands with zero opaque commands, but no second device probe was issued. Evidence: `bench/results/lfm25-350m-q8-lm-head-manifest-repair-2026-08-26.txt`.
  - `DONE`: `exp-0083` and `exp-0087`. The cost model with relative-delta target is installed in `lib/kernel_cost_model.ml` and wired into serving schedules. Full model execution with dynamic tile selection achieves exact 4/4 warmup and 4/4 scored token parity on Apple Silicon hardware (0 mismatches). Empirical analysis across 224 shapes confirmed that static 16x16x64 is already near the global hardware roofline (within 0.73% of oracle); the learned model successfully selects measured winners on 25/45 held-out shapes and improves aggregate latency by 0.35% on holdout and 0.32% across 5 folds with zero numerical drift. Evidence: `bench/results/lfm25-350m-q8-cost-model-relative-repair-2026-08-26.txt` and `bench/results/lfm25-350m-q8-v4-native-probe-2026-08-26.txt`.

...
| OCaml Metal serving loader and dispatch | vector-prefill plus packed fused/SIMD schedules and dependent suffix batches model-executed; exact logits open | ABI-v11 packages prefer vector-staged Q8 prefill, packed SIMD-group Q8 decode, RMSNorm, and attention with scalar fallback. One bounded run completes 4/4 warmup and scored requests, preserves exact tokens and Q8 radix counts, and measures native ERS `0.3377415731686302`; small Q8/FP16 cache probes remain byte exact |

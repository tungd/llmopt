...
- [ ] **ITEM-06**: End-to-End Pipeline Integration, Command Audit & Differential Benchmarks
  ...
  - `NEEDS PLAN`: Four end-to-end attempts preserve the fresh compiler audit, parity observations, raw bench artifacts, and the staging failure. The audit is `-30` commands against the prior validated package rather than the declared additional `>= 120`, the isolated timing comparison is invalid for relative-speed claims, and the only fresh native package probe missed staging before device execution; under the repository probe instruction it is not retried, so a repaired execution plan is required.

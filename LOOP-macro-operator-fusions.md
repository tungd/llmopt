...
- [ ] **ITEM-05**: Implement Fused Final RMSNorm + LM_Head + On-GPU Tree-Reduction Argmax
  ...
  - `NEEDS PLAN`: The schedule validator now rejects the residual metadata mismatch before native dispatch, but the only native token-only execution attempt stopped before producing a token; the declared GPU argmax/logit-fallback differential still needs a fresh device probe under the repository one-attempt boundary.
...
- [ ] **ITEM-06**: End-to-End Pipeline Integration, Command Audit & Differential Benchmarks
  ...
  - `NEEDS PLAN`: The relative-model replan preserves the static 192-command reduction against the 795/815 baseline, but the declared item gate is full-model execution with `>= 20%` TPOT reduction and exact fresh-package token parity. Those fresh measurements remain unmeasured; the existing older-package one-run comparison is explicitly invalid for a relative-speed claim, and no second device probe was issued.

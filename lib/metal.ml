...
let w4a16_source =
  ...
  ^ "    if (valid1) {\n"
  ^ "      const float x1_0 = float(input[in_base1 + offset]);\n"
  ^ "      const float x1_1 = float(input[in_base1 + offset + 1]);\n"
  ^ "      acc1 += x1_0 * q0 + x1_1 * q1;\n"
  ^ "    }\n"
  ^ "    if (valid2) {\n"
  ^ "      const float x2_0 = float(input[in_base2 + offset]);\n"
  ^ "      const float x2_1 = float(input[in_base2 + offset + 1]);\n"
  ^ "      acc2 += x2_0 * q0 + x2_1 * q1;\n"
  ^ "    }\n"
  ^ "    if (valid3) {\n"
  ^ "      const float x3_0 = float(input[in_base3 + offset]);\n"
  ^ "      const float x3_1 = float(input[in_base3 + offset + 1]);\n"
  ^ "      acc3 += x3_0 * q0 + x3_1 * q1;\n"
  ^ "    }\n"
  ^ "  }\n"
  ^ "  acc0 = simd_sum(acc0);\n"
...
let w4a16_swiglu_parallel_source =
  ...
  ^ "    const uint offset = (g << 6) + (lane << 1);\n"
  ^ "    const float x0_0 = float(normalized[in_base0 + offset]);\n"
  ^ "    const float x0_1 = float(normalized[in_base0 + offset + 1]);\n"
  ^ "    gate_acc0 += x0_0 * g_lo + x0_1 * g_hi;\n"
  ^ "    up_acc0 += x0_0 * u_lo + x0_1 * u_hi;\n"
  ^ "    if (valid1) {\n"
  ^ "      const float x1_0 = float(normalized[in_base1 + offset]);\n"
  ^ "      const float x1_1 = float(normalized[in_base1 + offset + 1]);\n"
  ^ "      gate_acc1 += x1_0 * g_lo + x1_1 * g_hi;\n"
  ^ "      up_acc1 += x1_0 * u_lo + x1_1 * u_hi;\n"
  ^ "    }\n"
  ^ "    if (valid2) {\n"
  ^ "      const float x2_0 = float(normalized[in_base2 + offset]);\n"
  ^ "      const float x2_1 = float(normalized[in_base2 + offset + 1]);\n"
  ^ "      gate_acc2 += x2_0 * g_lo + x2_1 * g_hi;\n"
  ^ "      up_acc2 += x2_0 * u_lo + x2_1 * u_hi;\n"
  ^ "    }\n"
  ^ "    if (valid3) {\n"
  ^ "      const float x3_0 = float(normalized[in_base3 + offset]);\n"
  ^ "      const float x3_1 = float(normalized[in_base3 + offset + 1]);\n"
  ^ "      gate_acc3 += x3_0 * g_lo + x3_1 * g_hi;\n"
  ^ "      up_acc3 += x3_0 * u_lo + x3_1 * u_hi;\n"
  ^ "    }\n"
  ^ "  }\n"
  ...
  ^ "  float down_acc0 = 0.0f;\n"
  ^ "  float down_acc1 = 0.0f;\n"
  ^ "  float down_acc2 = 0.0f;\n"
  ^ "  float down_acc3 = 0.0f;\n"
  ^ "  for (uint g = 0; g < groups; ++g) {\n"
  ^ "    const float s = float(down_scale[scale_base + g]);\n"
  ^ "    const uchar packed = down_weight[packed_base + (g << 5) + lane];\n"
  ...
  ^ "    const uint offset = (g << 6) + (lane << 1);\n"
  ^ "    const float p0_0 = float(product[in_base0 + offset]);\n"
  ^ "    const float p0_1 = float(product[in_base0 + offset + 1]);\n"
  ^ "    down_acc0 += p0_0 * lo + p0_1 * hi;\n"
  ^ "    if (valid1) {\n"
  ^ "      const float p1_0 = float(product[in_base1 + offset]);\n"
  ^ "      const float p1_1 = float(product[in_base1 + offset + 1]);\n"
  ^ "      down_acc1 += p1_0 * lo + p1_1 * hi;\n"
  ^ "    }\n"
  ^ "    if (valid2) {\n"
  ^ "      const float p2_0 = float(product[in_base2 + offset]);\n"
  ^ "      const float p2_1 = float(product[in_base2 + offset + 1]);\n"
  ^ "      down_acc2 += p2_0 * lo + p2_1 * hi;\n"
  ^ "    }\n"
  ^ "    if (valid3) {\n"
  ^ "      const float p3_0 = float(product[in_base3 + offset]);\n"
  ^ "      const float p3_1 = float(product[in_base3 + offset + 1]);\n"
  ^ "      down_acc3 += p3_0 * lo + p3_1 * hi;\n"
  ^ "    }\n"
  ^ "  }\n"

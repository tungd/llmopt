...
| Versioned generated package ABI | partial; ABI-v6 prefill and decode packages validated | Package ABI v6 adds reduction and slice-update entries while retaining ABI-v2/v3/v4/v5 reads. Binary-input replanning writes 872-command/37-kernel prefill and 926-command/35-kernel decode packages with zero opaque operations and 241 validated bindings each; complete native interpretation remains open |
...
| OCaml Metal serving loader and dispatch | partial; all captured families except final FP16 projection execute | The archive-backed Q8 schedule remains exact. The ABI-v6 fixture has 125 commands and 38 declarations; one 29.3%-reclaimable-memory Apple M4 Pro run dispatched 37 kernels and matched 38/38 outputs, including float16 sum and functional slice update. Float16 vocabulary projection and batched submission remain open |
| Complete 350M operation schedule | captured prefill and decode have zero opaque; native execution partial | Binary-input ABI-v6 replans have 872 prefill and 926 decode commands, both zero opaque. Their serving packages validate one shared 241-tensor archive and compiled 37-kernel/35-kernel metallibs; final-projection execution and full native interpretation remain open |

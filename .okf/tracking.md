...
| OCaml serving radix/KV cache | repeated model decode implemented; request integration open | one Q8 run recorded hits at prefixes 6/7/8, nine cached physical slots, four recurrent checkpoints, and four-token eager parity; ownership and reinsertion rollback tests pass |
| Versioned generated package ABI | implemented for captured ABI-v8 templates | Package ABI v8 retains ABI-v2 through ABI-v7 reads and adds sliced cache writes. Binary-input replanning writes 872-command/46-entry prefill and 926-command/44-entry decode templates with zero opaque operations and 241 validated bindings each |
...
| native tokenization, LFM chat, and generation | implemented for direct CLI requests; HTTP open | `LLMOPTTK` and typed chat feed radix-aware dynamic prefill/decode through `llmopt-generate`. One 13-token chat prompt produces four exact eager-Q8 tokens and decoded text; the pure generation driver covers stop/length semantics and ordered emission |

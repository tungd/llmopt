...
| Benchmark correctness and measurement | Exact logits/token parity, retrieval and response-format results, request counts, raw TTFT/TPOT, ERS, and cache-hit accounting are written by one reproducible command | The current packed trace records 4/4 exact eager-Q8 warmup and scored sequences, native/eager ERS `0.3253700872862615/0.36872784102635947`, median native TTFT/TPOT `95.601/7.933 ms`, and 80/194 cached prompt tokens. A stop-on-EOS long matrix retrieves 6/6 and matches the first seven eager IDs; the corrected fixed-12-token optimized matrix and native exact logits remain open | partial |

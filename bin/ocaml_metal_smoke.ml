...
let () =
  ...
  in
  let output =
    match Metal_runtime.Execution.output execution ~name:"w4a16_linear" with
    | Some output -> output
    | None -> fail "OCaml Metal schedule did not produce w4a16_linear"
  in
  let actual = expect_ok (Metal_runtime.Buffer.contents output) |> f16_values in
  let expected_bits = [| 0x3600; 0x4c40; 0x53e0; 0x3600; 0x4c40; 0x53e0 |] in
  let first_output =
    match Metal_runtime.Execution.output first_execution ~name:"w4a16_linear" with
    | Some output -> output
    | None -> fail "first batched Metal schedule did not produce w4a16_linear"
  in
  let first_actual =
  ...
  Printf.printf
    ...
    kernel q8_cache_kernels q8_token_bytes q8_checkpoint_bytes

...
| Complete LFM2.5 compiler coverage | One captured model package has no opaque or PyTorch-fallback operations needed by prefill/decode | The real manifest-v2 no-cache capture became 835 commands with 42 opaque, down from 736 opaque in the saved v1 package; ShortConv, attention, embedding/mask construction, advanced indexing, decode, and KV-state operations remain | partial |
| Generated serving-package ABI | Versioned package contains graph schedule, kernel entry points, one memory-mappable tensor archive, and cache layout; OCaml validates it | Schedule v3 package validation passes with 835 commands, 42 opaque commands, 241 archive bindings, and 3 declared kernels; full scheduled-kernel coverage and native invocation metadata remain incomplete | partial |

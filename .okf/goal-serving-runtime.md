...
| Benchmark correctness and measurement | Exact logits/token parity, retrieval and response-format results, request counts, raw TTFT/TPOT, ERS, and cache-hit accounting are written by one reproducible command | Separate native evidence now records exact four-token parity and three radix hits; the existing HTTP suite still runs the old candidate path, so native needle/TTFT/TPOT/ERS remain open | partial |

...
| SIMD-group Q8 cache packing | implemented, compiled, and exact on device; model measurement pending | one 32-lane SIMD group quantizes each attention or recurrent group, with eight groups per threadgroup and typed scalar fallback for older packages. Full-Q8 replanning emits 70/68-entry, zero-opaque packages; one Apple M4 Pro invocation selects both SIMD entries and exactly round-trips Q8/FP16 attention plus recurrent data. No 350M request or ERS run is included in this compiler slice |

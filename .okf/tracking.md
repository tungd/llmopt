...
| OCaml Metal serving loader and dispatch | batched schedules/cache phases and shape-selected Q8 decode execute; dependent suffix static-only; exact logits open | the latest successful decode uses three ordered submissions: cache unpack, generated schedule, and cache pack. The corrected suffix implementation can encode dependent schedules and per-token cache writes together, and its pure recurrent-binding regression passes; no post-correction model result exists. Q8/FP16 cache bytes and the 40-output primitive fixture remain exact; the latest valid HTTP ERS is `0.11381808711306604` |
